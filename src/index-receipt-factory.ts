...
// Real L4 build provenance, written by scripts/gen-provenance.mjs at build time.
// These fill the v0.2 receipt's build-provenance fields (which already exist in
// the canon) with real values instead of inline sentinels. No schema/canon/golden
// change — only the field VALUES change from placeholders to real build context.
import { BUILD_PROVENANCE } from './provenance.gen.js';
...
export async function buildDemoReceipt(
  ...
  const draft: ExecutionReceiptV02Draft = {
    ...
    // ─── Build provenance: the Worker's REAL L4 build context (was sentinels) ─
    // Sourced from src/provenance.gen.ts (generated by scripts/gen-provenance.mjs
    // at build time). These are the SAME mandatory v0.2 fields the golden carries
    // — only the values are now real (a real git commit, the sha256 of the built
    // bundle, and the sha256 of the bundle's DSSE in-toto attestation) instead of
    // the inline sentinels. artifact_type stays the existing `generic-binary`.
    artifact_digest: BUILD_PROVENANCE.artifactDigest,
    artifact_type: BUILD_PROVENANCE.artifactType,
    build_id: BUILD_PROVENANCE.buildId,
    git_commit: BUILD_PROVENANCE.gitCommit,
    attestation_digest: BUILD_PROVENANCE.attestationDigest,
    prev_hash: null,
  };
